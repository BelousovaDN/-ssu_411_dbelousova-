...
public class Calcs {
    ...
    public static ArrayList<String> pols(String mes1) throws Exception {
        ...
        System.out.println("Польская запись: " + ssEx);
        return ssEx;
    }
    ...
    public static Double calculate(ArrayList<String> mes) throws Exception {
        ...
        for (int i=0; i< mes.size(); i++){
            if (checkString(mes.get(i))){
                ...
                if (steak.size() < 2) {
                    throw new Exception("Неверное количество данных для вычисления выражения. Проверьте корректность введенных данных!");
                }
